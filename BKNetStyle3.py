...
def BKNetModel(x):
    ...
    y_smile_conv = _FC('smile_softmax', smile_fc2, 2, keep_prob, 'linear')
    ...
    y_gender_conv = _FC('gender_softmax', gender_fc2, 2, keep_prob, 'linear')
...
def selective_loss(y_smile_conv, y_emotion_conv, y_gender_conv, y_, mask):
    ...
    smile_cross_entropy = tf.reduce_sum(
        tf.reduce_sum(-y_smile * tf.log(tf.clip_by_value(tf.nn.softmax(y_smile_conv), 1e-10, 1.0)),
                      axis=1) * smile_mask) / tf.clip_by_value(tf.reduce_sum(smile_mask), 1, int(1e9))

    emotion_cross_entropy = tf.reduce_sum(
        tf.reduce_sum(-y_emotion * tf.log(tf.clip_by_value(tf.nn.softmax(y_emotion_conv), 1e-10, 1.0)),
                      axis=1) * emotion_mask) / tf.clip_by_value(tf.reduce_sum(emotion_mask), 1, int(1e9))

    gender_cross_entropy = tf.reduce_sum(
        tf.reduce_sum(-y_gender * tf.log(tf.clip_by_value(tf.nn.softmax(y_gender_conv), 1e-10, 1.0)),
                      axis=1) * gender_mask) / tf.clip_by_value(tf.reduce_sum(gender_mask), 1, int(1e9))
